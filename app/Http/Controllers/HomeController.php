...
class HomeController extends Controller
{
    ...
    /**
     * Get dashboard statistics for orders and bookings
     */
    public function getDashboardStats()
    {
        try {
            if (request()->ajax()) {
                ...
                $startDateTime = \Carbon\Carbon::parse($start)->startOfDay();
                $endDateTime = \Carbon\Carbon::parse($end)->endOfDay();
                ...
                $deniedBookingsCount = 0;
                $cancelledBookingsCount = 0;

                // Check if Order model exists and has data
                if (class_exists('\App\Models\Order')) {
                    try {
                        // Debug: Check if orders table exists and has data
                        \Log::info('Checking Order model...');
                        
                        // First, let's see what statuses exist in the orders table
                        $orderStatuses = \App\Models\Order::select('status')->distinct()->pluck('status')->toArray();
                        \Log::info('Available order statuses:', $orderStatuses);
                        
                        // Check if business_id column exists
                        try {
                            $testQuery = \App\Models\Order::where('business_id', $business_id)->first();
                            \Log::info('business_id column exists and has data');
                        } catch (\Exception $e) {
                            \Log::info('business_id column might not exist: ' . $e->getMessage());
                            // Try without business_id
                            $business_id = null;
                        }
                        
                        // Get pending orders count and total
                        $pendingOrdersQuery = \App\Models\Order::when($business_id, function($query) use ($business_id) {
                        ...
                        $pendingOrdersTotal = $pendingOrdersQuery->sum('amount');
                        
                        \Log::info('Pending orders query:', [
                            'business_id' => $business_id,
                            'status' => 'pending',
                            'start' => $start,
                            'end' => $end,
                        ...
                        // Get completed orders count (status is 'confirmed' not 'completed')
                        $confirmedOrdersQuery = \App\Models\Order::when($business_id, function($query) use ($business_id) {
                                return $query->where('business_id', $business_id);
                            })
                        ...
                        $completedOrdersCount = $confirmedOrdersQuery->count();

                        $deniedOrdersQuery = \App\Models\Order::when($business_id, function($query) use ($business_id) {
                                return $query->where('business_id', $business_id);
                            })
                            ->whereRaw('LOWER(status) = ?', ['denied'])
                            ->whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end);
                        \Log::info('Denied orders query SQL: ' . $deniedOrdersQuery->toSql());
                        \Log::info('Denied orders bindings: ', $deniedOrdersQuery->getBindings());
                        $deniedOrdersCount = $deniedOrdersQuery->count();
                        \Log::info('Denied orders count: ' . $deniedOrdersCount);

                        $cancelledOrdersQuery = \App\Models\Order::when($business_id, function($query) use ($business_id) {
                                return $query->where('business_id', $business_id);
                            })
                            ->whereIn(DB::raw('LOWER(status)'), ['cancelled', 'canceled'])
                            ->whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end);
                        \Log::info('Cancelled orders query SQL: ' . $cancelledOrdersQuery->toSql());
                        \Log::info('Cancelled orders bindings: ', $cancelledOrdersQuery->getBindings());
                        $cancelledOrdersCount = $cancelledOrdersQuery->count();
                        \Log::info('Cancelled orders count: ' . $cancelledOrdersCount);
                        
                        \Log::info('Confirmed orders query:', [
                            'business_id' => $business_id,
                            'status' => 'confirmed',
                            'start' => $start,
                            'end' => $end,
                    ...
                    \Log::info('Order model does not exist');
                }

                // Check if Booking model exists and has data
                if (class_exists('\App\Models\Booking')) {
                    try {
                        // Get pending bookings count
                        $pendingBookingsQuery = \App\Models\Booking::whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end)
                            ->whereRaw('LOWER(status) = ?', ['pending']);
                        \Log::info('Pending bookings query SQL: ' . $pendingBookingsQuery->toSql());
                        \Log::info('Pending bookings bindings: ', $pendingBookingsQuery->getBindings());
                        $pendingBookingsCount = $pendingBookingsQuery->count();
                        \Log::info('Pending bookings count: ' . $pendingBookingsCount);

                        // Get accepted bookings count
                        $acceptedBookingsQuery = \App\Models\Booking::whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end)
                            ->whereRaw('LOWER(status) = ?', ['accepted']);
                        \Log::info('Accepted bookings query SQL: ' . $acceptedBookingsQuery->toSql());
                        \Log::info('Accepted bookings bindings: ', $acceptedBookingsQuery->getBindings());
                        $acceptedBookingsCount = $acceptedBookingsQuery->count();
                        \Log::info('Accepted bookings count: ' . $acceptedBookingsCount);

                        $deniedBookingsQuery = \App\Models\Booking::whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end)
                            ->whereRaw('LOWER(status) = ?', ['denied']);
                        \Log::info('Denied bookings query SQL: ' . $deniedBookingsQuery->toSql());
                        \Log::info('Denied bookings bindings: ', $deniedBookingsQuery->getBindings());
                        $deniedBookingsCount = $deniedBookingsQuery->count();
                        \Log::info('Denied bookings count: ' . $deniedBookingsCount);

                        $cancelledBookingsQuery = \App\Models\Booking::whereDate('created_at', '>=', $start)
                            ->whereDate('created_at', '<=', $end)
                            ->whereIn(DB::raw('LOWER(status)'), ['cancelled', 'canceled']);
                        \Log::info('Cancelled bookings query SQL: ' . $cancelledBookingsQuery->toSql());
                        \Log::info('Cancelled bookings bindings: ', $cancelledBookingsQuery->getBindings());
                        $cancelledBookingsCount = $cancelledBookingsQuery->count();
                        \Log::info('Cancelled bookings count: ' . $cancelledBookingsCount);
                    } catch (\Exception $e) {
                        \Log::error('Booking query error: ' . $e->getMessage());
                    }
                }
                ...
                \Log::info('Dashboard stats results:', [
                    ...
                    'denied_orders_count' => $deniedOrdersCount,
                    'cancelled_orders_count' => $cancelledOrdersCount,
                    'pending_bookings_count' => $pendingBookingsCount,
                    'accepted_bookings_count' => $acceptedBookingsCount,
                    'denied_bookings_count' => $deniedBookingsCount,
                ...
                return response()->json([
                    'pending_orders_count' => $pendingOrdersCount,
                    'pending_orders_total' => number_format($pendingOrdersTotal, 2),
                    'completed_orders_count' => $completedOrdersCount,
                    'denied_orders_count' => $deniedOrdersCount,
                    'cancelled_orders_count' => $cancelledOrdersCount,
                    'pending_bookings_count' => $pendingBookingsCount,
            ...
            return response()->json([
                ...
                'denied_orders_count' => 0,
                'cancelled_orders_count' => 0,
                'pending_bookings_count' => 0,
                'accepted_bookings_count' => 0,
                'denied_bookings_count' => 0,
