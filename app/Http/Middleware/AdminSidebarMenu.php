...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //Billboard
            $menu->url('/billboard', 'Billboard', ['icon' => 'fa fas fa-th-large', 'active' => request()->segment(1) == 'billboard'])->order(6);

            //Services dropdown
            $menu->dropdown('Service', function ($sub) {
                $sub->url('/services', 'Services', ['icon' => 'fa fas fa-wrench', 'active' => request()->is('services*')]);
                $sub->url('/sub-services', 'Sub Services', ['icon' => 'fa fas fa-tools', 'active' => request()->is('sub-services*')]);
                $sub->url('/service-prices', 'Service Prices', ['icon' => 'fa fas fa-dollar-sign', 'active' => request()->is('service-prices*')]);
            }, ['icon' => 'fa fas fa-cogs'])->order(7);

            //Shop dropdown
            $menu->dropdown('D-WORLD', function ($sub) {
                $sub->url('/orders', 'Orders', ['icon' => 'fa fas fa-shopping-cart', 'active' => request()->is('orders*')]);
                $sub->url('/booking', 'Booking Services', ['icon' => 'fa fas fa-calendar-check', 'active' => request()->is('booking*')]);
            }, ['icon' => 'fa fas fa-store'])->order(8);

            //Car Maintenance
            $menu->url('/car-maintenance', 'Car Maintenance Record', ['icon' => 'fa fas fa-car', 'active' => request()->segment(1) == 'car-maintenance'])->order(9);
